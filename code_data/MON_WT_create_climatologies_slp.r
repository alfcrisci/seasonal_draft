...
for ( j in 1:12) {
                       ...
                       namem_rel=list.files(path=dirdataMON_stacks,pattern=paste0(vars[i],".*_",j,".nc"),recursive = T,full.names = F)
                        if (vars[i]=="pp_pct09"" || vars[i]=="pp_san09") {namem=list.files(path=dirdataMON_stacks,pattern=paste0("pp",".*_",j,".nc"),recursive = T,full.names = T);
                                                                                             namem_rel=list.files(path=dirdataMON_stacks,pattern=paste0("pp",".*_",j,".nc"),recursive = T,full.names = F)
                                                                                             };
                       ...
                       for ( z in 1:9) {
                                          ...
                                          if ( vars[i]=="pp_pct09"" ) { name=list.files(path=dirdataWT_stacks,pattern=paste0("*","pct09",".*_",j,"_",z),recursive = T,full.names = T);
                                                                                  name_rel=list.files(path=dirdataWT_stacks,pattern=paste0("*","pct09",".*_",j,"_",z),,recursive = T,full.names = F)

                                                         };
                                          if ( vars[i]=="pp_san09" ) { name=list.files(path=dirdataWT_stacks,pattern=paste0("*","san09",".*_",j,"_",z),recursive = T,full.names = T);
                                                                                name_rel=list.files(path=dirdataWT_stacks,pattern=paste0("*","san09",".*_",j,"_",z),recursive = T,full.names = F)
